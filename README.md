...
Furthermore, the ```csv``` folder contains the outputs of the simulations generated by ```wifi_1ch.cc```, ```wifi_static.cc``` and ```wifi_dynamic.cc```, in form of .csv files.

In order to parse relevant information from them, three python scripts are available:
